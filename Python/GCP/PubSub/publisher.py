...
def publish_messages_with_source(project_id: str, topic_id: str, source_msgs: str) -> None:
    """Publishes a message to a Pub/Sub topic using a payload/file."""
    # [START pubsub_publish_with_source]
    from google.cloud import pubsub_v1
    ...
    if Path(source_msgs).is_file():
        msg = Path(source_msgs).read_text()
    else:
        msg = source_msgs

    data_str = f"{msg.strip()}"
    # Data must be a bytestring
    data = data_str.encode("utf-8")
    # When you publish a message, the client returns a future.
    future = publisher.publish(topic_path, data)
    ...
    # [END pubsub_publish_with_source]
...
if __name__ == "__main__":  # noqa: C901
    ...
    publish_source_parser = subparsers.add_parser("publish-with-source", help=publish_messages_with_source.__doc__)
    publish_source_parser.add_argument("topic_id")
    publish_source_parser.add_argument("source_msgs")
    ...
    elif args.command == "publish-with-source":
        publish_messages_with_source(args.project_id, args.topic_id, args.source_msgs)
